...
router.get('/timeline/friends', passport.authenticate('jwt', {session : false}), async (req : Request, res : Response) => {
...
router.get('/timeline/public', passport.authenticate('jwt', {session : false}), async (req : Request, res : Response) => {
    const user : any  = req.user;
    try{
        // const relationshipResult : any = await RelationshipModel.find({ $and : [{ $or : [{user1 : user._id}, {user2 : user._id}]}, {status : 1}]}).select('user1 user2').exec();
        // console.log(relationshipResult);
        ...
        // console.log(friends);
        //console.log(Array.isArray(relationshipResult));
        const momentResult : any = await MomentModel.find({visibility : 1}).populate('ownerId', '-_id username firstname lastname profileImage').exec();
        console.log(momentResult);
        // const r = await MomentModel.findOne({ownerId : "5edb6123823146361c03ddd0"}).exec();
        // console.log(r)
        const responseArray = [];
